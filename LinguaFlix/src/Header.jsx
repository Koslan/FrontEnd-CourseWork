...
function Header({ onSearch, results, isVisible }) {
  const [showModal, setShowModal] = useState(false); // Створення стейту showModal
  ...
  const handleUserClick = () => {
    if (user.isLoggedIn) {
      // Користувач залогінений, показати HeaderUser
      setShowModal(false);
    } else {
      // Користувач не залогінений, показати форму реєстрації
      setShowModal(true);
    }
  };
  ...
  return (
    <header className="header">
      ...
      <div className="login" onClick={handleLoginIconClick}>
        {!user.isLoggedIn && (
        ...
        {showModal && <Login showModal={showModal} />} {/* Додавання компонента Login */}
      </div>
    </header>
  );
}
